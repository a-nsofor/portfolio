...
side:"header-leftpane"===a||"header-minbar"===a?"left":"right",hasOverlay:!("header-slide-out"===a||"header-rightpane"===a)});if(void 0!=l.m_m_expand)c.on("sidemenushow.themify",function(a){if("#mobile-menu"===a){a=e.querySelectorAll("#main-nav>li.has-sub-menu");for(let b=a.length-1;-1<b;b--)a[b].className+=" toggle-on"}},!0);if("header-top-widgets"===a){const a=this,e=function(){a.bodyCl.contains("mobile_menu_active")?d(".header-widget-full > .header-widget-inner").appendTo("#mobile-menu"):d("#mobile-menu .header-widget-inner").prependTo(".header-widget-full")};
e();c.on("tfsmartresize",function(a){a&&e(a)})}},backToTop(){const a=e.getElementsByClassName("back-top"),b=this.isFullPageScroll,d=b?null:e.querySelector(".back-top-float:not(.footer-tab)");if(d){var f=["scroll"];const a=function(){d.classList.toggle("back-top-hide",10>this.scrollY)};c.isTouch&&(f.push("touchstart"),f.push("touchmove"));for(let b=f.length-1;-1<b;--b)h.addEventListener(f[b],a,{passive:!0})}if(a[0]){const d=function(a){const b=e.getElementById("footerwrap");if(b){b.classList.remove("tf_hide");
